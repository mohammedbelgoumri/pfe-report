...
La plus simple est d'assimiler chaque mot à un token.
...
remplaçant les autres par un token spécial \texttt{[UNK]} (\foreignlanguage{english}{unknown}),
mais cela a pour conséquence de perdre l'information sur les mots rares.
Dans un contexte de correction d'erreurs, celles-ci sont intrinsèquement rares. 
...
Pour construire le vocabulaire, \gls{bpe} commence par découper les mots en caractères (les tokens initiaux).
Ensuite, il regroupe le couple de tokens le plus fréquent en un seul token, 
remplaçant toutes leurs occurrences par ce dernier.
Cette opération est répétée jusqu'à ce que le nombre de tokens atteigne la valeur désirée
